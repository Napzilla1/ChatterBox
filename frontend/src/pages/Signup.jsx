...
const Signup = () => {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            const res = await axios.post('http://localhost:8000/api/auth/signup', inputs, {
            ...
            <div className="w-full p-6 rounded-lg shadow-md bg-white">
                ...
                <form onSubmit={handleSubmit} className="mt-4">
                    <div>
                        ...
                        <input type="text" placeholder="johndoe" className="w-full input input-bordered h-10 px-3 py-2 border rounded-md"
                            value={inputs.username} onChange={(e) => setInputs({ ...inputs, username: e.target.value })}
                        />
                    </div>
                    <div className="mt-4">
                        <label className="label p-2"><span className="text-base label-text">Email</span></label>
                        <input type="email" placeholder="johndoe@gmail.com" className="w-full input input-bordered h-10 px-3 py-2 border rounded-md"
                    ...
                    <div>
                        <button className="w-full bg-blue-500 text-white p-2 rounded-md mt-4 hover:bg-blue-600">Sign Up</button>
                    </div>
                </form>
            </div>
        </div>
